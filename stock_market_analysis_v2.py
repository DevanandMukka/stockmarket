...
if uploaded_file is None:
    ...
    if next_tc >= next_R3 >= next_bc:
        ...
        <span style="font-size:24px; color:{'#404040' if second_fact else '#dc2626'};">2nd condition satisfied: {"Yes" if second_fact else "No"}</span>
        </b>
        """
...
# ========================================================== ADDED on MARCH 14
# === MULTI TIMEFRAME DPZ OVERLAP (DAY vs WEEK vs MONTH) ===
...
df_mtf["Date"] = pd.to_datetime(df_mtf["Date"])
df_mtf = df_mtf.sort_values("Date")

def calculate_next_levels(data):

    last = data.iloc[-1]

    pivot = (last["High"] + last["Low"] + last["Close"]) / 3
    bc = (last["High"] + last["Low"]) / 2
    tc = pivot + (pivot - bc)
    ...
    R3 = last["Close"] + rng * 1.1 / 4
    R4 = last["Close"] + rng * 1.1 / 2
    S3 = last["Close"] - rng * 1.1 / 4
    S4 = last["Close"] - rng * 1.1 / 2
...
# --- DAILY LEVELS
daily_levels = calculate_next_levels(df_mtf)

# --- WEEKLY LEVELS
df_week = df_mtf.set_index("Date").resample("W-FRI").agg({
    "High":"max","Low":"min","Close":"last"
}).dropna().reset_index()

weekly_levels = calculate_next_levels(df_week)

# --- MONTHLY LEVELS
df_month = df_mtf.set_index("Date").resample("M").agg({
    "High":"max","Low":"min","Close":"last"
}).dropna().reset_index()

monthly_levels = calculate_next_levels(df_month)


def check_overlap(level1, level2, ref_price, tolerance=0.001):
    return abs(level1 - level2) <= ref_price * tolerance


price_ref = last_day_data["Close"]

mtf_messages = []

# Daily vs Weekly
if check_overlap(daily_levels["r1"], weekly_levels["R3"], price_ref):
    mtf_messages.append("Daily R1 overlapping Weekly Camarilla R3 → Strong Resistance Confluence")

if check_overlap(daily_levels["s1"], weekly_levels["S3"], price_ref):
    mtf_messages.append("Daily S1 overlapping Weekly Camarilla S3 → Strong Support Confluence")


# Daily vs Monthly
if check_overlap(daily_levels["r2"], monthly_levels["R3"], price_ref):
    mtf_messages.append("Daily R2 overlapping Monthly Camarilla R3 → Institutional Resistance Zone")

if check_overlap(daily_levels["s2"], monthly_levels["S3"], price_ref):
    mtf_messages.append("Daily S2 overlapping Monthly Camarilla S3 → Institutional Support Zone")


# Weekly vs Monthly
if check_overlap(weekly_levels["r1"], monthly_levels["R3"], price_ref):
    mtf_messages.append("Weekly R1 overlapping Monthly Camarilla R3 → Major Resistance Zone")

if check_overlap(weekly_levels["s1"], monthly_levels["S3"], price_ref):
    mtf_messages.append("Weekly S1 overlapping Monthly Camarilla S3 → Major Support Zone")


if mtf_messages:

    mtf_html = "<br>".join(mtf_messages)
    ...
    <div style="text-align:center;font-size:22px;font-weight:bold;
        ...
        border:1px solid #06b6d4;margin-top:15px;">

        <div style="font-size:26px;color:#0891b2;margin-bottom:10px;">
        🌍 Multi Timeframe DPZ Confluence
        </div>
    ...
    st.info("No multi-timeframe DPZ confluence detected between Daily, Weekly, and Monthly levels.")
